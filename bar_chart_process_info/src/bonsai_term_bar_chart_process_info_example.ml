...
let make_view ~(dimensions : Dimensions.t Bonsai.t) (local_ graph) =
  ...
  let bars =
    ...
    List.map process_info ~f:(fun { name; cpu } ->
      { Bonsai_term_chart.Bar_spec.label = Some name
      ; value = Percent.to_percentage cpu
      ; color = None
      })
  in
  let y_labels_config =
    Bonsai_term_chart.Scalar_axis_labels_config.Every_x_units
      { x = 25.; make_label_string = (fun x -> Float.to_string_hum ~decimals:0 x ^ "%") }
  in
  let%arr { height; width } = dimensions
  and bars in
  Bonsai_term_chart.Bar_chart.view
